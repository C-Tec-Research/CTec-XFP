...
namespace Xfp.DataTypes.PanelData
{
    ...
    public static string EquationParseResultToString(EquationParseResult result, int numItems = 0)
        ...
        /// <returns></returns>
        private EquationParseResult parseIntList(string stringValue, out List<Range> rangeList)
        {
            rangeList = new();
            ...
            foreach (var seg in splitSegments(stringValue))
            {
                ...
                if (int.TryParse(range[0], out rangeStart))
                {
                    ...
                    rangeList.Add(new() { StartValue = rangeStart, EndValue = rangeEnd });
        ...
        /// <returns></returns>
        private EquationParseResult parseCharList(string stringValue, out List<Range> rangeList)
        {
            rangeList = new();
            Equation = new();
